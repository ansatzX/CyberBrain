...
> Scope: this document describes only the Cyberbrain Codex adapter. Pi packaging and lifecycle are documented in [PI_SUPPORT.md](PI_SUPPORT.md).
>
...
├── agents/                  # 可选：项目自带的 agent TOML 源文件；插件加载器不会直接识别
...
因为插件系统不原生支持 agent role，推荐做法是把“插件发布”和“agent 安装”拆开：

1. 在插件里放 `agents/*.toml`（Codex agent role 格式）
2. 提供一个**显式安装器**，把这些 `.toml` copy 到 `~/.codex/agents/`
3. 安装器写一个 ownership manifest，后续 `doctor` / `uninstall` 只处理自己装进去的文件
4. 用显式安装器把 agent TOML copy 到 `~/.codex/agents/`
...
**局限**：
- Codex 没有 `on_install` / `on_uninstall` 事件，所以安装器需要用户显式运行
- Marketplace 安装插件本身不会自动把 agent role 注册进 `~/.codex/agents/`
...
- 确认后删除
- 不触碰用户自己的普通 `.toml` 文件
- 如果是历史 symlink 安装，建议先运行显式安装器迁移，再用这个脚本清理残留 symlink
