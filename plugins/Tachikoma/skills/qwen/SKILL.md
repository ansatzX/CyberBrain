...
```bash
# One-shot prompt
qwen -p "Review src/ for bugs" 2>/dev/null

# Plan only mode
qwen -p "Analyze the codebase architecture" --approval-mode plan 2>/dev/null

# Yolo mode (auto-approve everything)
qwen -p "Fix bug in file.py. Apply now." --yolo 2>/dev/null

# Continue last session
qwen --continue -p "What else can you improve?" 2>/dev/null

# If redirection fails, wrap in bash -lc
bash -lc 'qwen -p "prompt" 2>/dev/null'
```
